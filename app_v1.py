...
def get_website_content(website_url, timeout=20, retries=3):
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            pass  # Suppress all warnings/errors
    return ""
...
def extract_manufacturer_info(company_name):
    ...
    prompt = f"Find the official website and LinkedIn page for the company named '{company_name}'. Return both URLs if available."
    answer, _ = get_perplexity_answer(prompt)
    linkedin_url, website_url = None, None
    for line in answer.splitlines():
        if "linkedin.com" in line:
            linkedin_url = line.strip()
        if line.startswith("http") and "linkedin.com" not in line:
...
with search_tab:
    ...
    if search_mode == "General Search":
        ...
        reload_clicked = st.button("Reload Results 🔄")
        if reload_clicked:
            st.session_state['last_general_search_results'] = {}
            st.session_state['last_specific_company_summary'] = None
            st.session_state['suppliers_for_analysis'] = []
            st.session_state['analysis_results'] = {}
            st.session_state['analyzed_suppliers'] = set()
            st.success("Results cleared. Please perform a new search.")
        ...
        if st.session_state['last_general_search_results']:
            ...
            get_diff_results_btn = st.button("Get Different Results", key="get_different_results_btn")
            if get_diff_results_btn:
                ...
                if not user_prompt:
                    ...
                    with st.spinner("Generating different search queries and results..."):
                        import random
                        _, search_terms = parse_user_prompt_for_search(user_prompt)
